...
#define getpixu(x, y) ((x) >= width || (y) >= height ? 0xff : ary[(x) + (y) * width])
...
#define MAXITER 32
    ...
    for (iter = 0; iter < MAXITER; iter++) {
        ...
        fprintf(stderr,
            ...
            black, white, global_cutlevel, global_cutlevel, fill_global_cutlevel, fill_global_cutlevel);
        if (global_cutlevel == lastcutlevel)
            break; /* Stable state reached */
    }
    if (iter == MAXITER)
        fprintf(stderr,
            "Warning: cutting point analysis didn't "
...
static void diag_scan(int* outx, int* outy, int xin, int yin, int dx, unsigned dy)
{
    int xbegin, x, y;
    unsigned ctr, len;
...
static void find_corners(void)
{
    ...
    fprintf(stderr,
        "One bit is %G horizontal pixels and %G "
        "vertical pixels.\n",
        hpixel, vpixel);
    ...
    pixelhx = ((double)corners[1][0] + (double)corners[3][0] - (double)corners[0][0] - (double)corners[2][0]) / 2;
    pixelhy = ((double)corners[1][1] + (double)corners[3][1] - (double)corners[0][1] - (double)corners[2][1]) / 2;
    pixelvx = ((double)corners[2][0] + (double)corners[3][0] - (double)corners[0][0] - (double)corners[1][0]) / 2;
    pixelvy = ((double)corners[2][1] + (double)corners[3][1] - (double)corners[0][1] - (double)corners[1][1]) / 2;
    ...
    fprintf(stderr,
        "Input horizontal pixel vector %G,%G, vertical %G,%G."
        " skew %G deg, perpendicularity %G deg.\n",
        pixelhx, pixelhy, pixelvx, pixelvy, normalize_angle(angle(pixelhx, -pixelhy) + angle(pixelvx, -pixelvy) + 90) / 2,
    ...
    fprintf(stderr,
        "Upper corners at %lu, %lu and %lu, %lu,\n"
        "lower corners at %lu, %lu and %lu, %lu.\n"
        "Cross half for searching is %d x %d input pixels.\n",
        corners[0][0], corners[0][1], corners[1][0], corners[1][1], corners[2][0], corners[2][1], corners[3][0],
        corners[3][1], chalf, chalf);
}
...
static float get_pixel_interp(double x, double y)
{
    ...
    return bilinearf(getpixu(xi, yi), getpixu(xi + 1, yi), getpixu(xi, yi + 1), getpixu(xi + 1, yi + 1), x - xi, y - yi);
}
...
static float pixel_correct_sample(double x, double y)
{
    ...
    avg = get_pixel_interp(PSHIFTX(x, -hdist, 0), PSHIFTY(y, -hdist, 0));
    avg += get_pixel_interp(PSHIFTX(x, hdist, 0), PSHIFTY(y, hdist, 0));
    avg += get_pixel_interp(PSHIFTX(x, 0, vdist), PSHIFTY(y, 0, vdist));
    avg += get_pixel_interp(PSHIFTX(x, 0, -vdist), PSHIFTY(y, 0, -vdist));
    avg /= 4;
...
static float cross_correl_search(int xpos, int ypos)
{
    ...
    /* Middles of sides */
    sum += 2 * (getsearch(xpos - chalf, ypos) + getsearch(xpos + chalf, ypos) + getsearch(xpos, ypos - chalf) +
...
static void cross_stats(unsigned cx, unsigned cy)
{
    ...
    for (xoff = -hpixelhalf; xoff <= hpixelhalf; xoff++)
        for (yoff = -vpixelhalf; yoff <= vpixelhalf; yoff++) {
            val = get_pixel_interp(PSHIFTX(centerx, xoff, yoff), PSHIFTY(centery, xoff, yoff));
...
#define HALFRANGE                                                  \
    (0.5 / FINESTEP) /* 0.5 means 0.5 of small input pixel - no    \
    ...
    for (xoff = -HALFRANGE; xoff <= HALFRANGE; xoff++)
        for (yoff = -HALFRANGE; yoff <= HALFRANGE; yoff++) {
            ...
            result = cross_correl(PSHIFTX(xmax, (double)xoff * FINESTEP, (double)yoff * FINESTEP),
                PSHIFTY(ymax, (double)xoff * FINESTEP, (double)yoff * FINESTEP));
            if (result > max) {
                max     = result;
                xoffmax = xoff;
                yoffmax = yoff;
...
static void sync_crosses(void)
{
    ...
    rightx = ((double)corners[1][0] + corners[3][0] - corners[0][0] - corners[2][0]) / 2 * CPITCH / WIDTH;
    righty = ((double)corners[1][1] + corners[3][1] - corners[0][1] - corners[2][1]) / 2 * CPITCH / WIDTH;
    downx  = ((double)corners[2][0] + corners[3][0] - corners[0][0] - corners[1][0]) / 2 * CPITCH / format_height;
    downy  = ((double)corners[2][1] + corners[3][1] - corners[0][1] - corners[1][1]) / 2 * CPITCH / format_height;
    ...
    crosses[0][0][0] = bilinear(corners[0][0], corners[1][0], corners[2][0], corners[3][0],
        (double)(BORDER + CHALF) / WIDTH, (double)(BORDER + CHALF) / format_height);
    crosses[0][0][1] = bilinear(corners[0][1], corners[1][1], corners[2][1], corners[3][1],
        (double)(BORDER + CHALF) / WIDTH, (double)(BORDER + CHALF) / format_height);

    fprintf(stderr,
        "Finding crosses (%u lines), numbers indicate "
...
static void bit_coord(double* xout, double* yout, float* cutlevel, int x, int y)
{
    ...
    xd = bilinear(crosses[cx][cy][0], crosses[cx + 1][cy][0], crosses[cx][cy + 1][0], crosses[cx + 1][cy + 1][0], xrem,
        yrem);
    yd = bilinear(crosses[cx][cy][1], crosses[cx + 1][cy][1], crosses[cx][cy + 1][1], crosses[cx + 1][cy + 1][1], xrem,
        yrem);
    if (cutlevel) {
        *cutlevel = bilinearf(cutlevels[cx][cy], cutlevels[cx + 1][cy], cutlevels[cx][cy + 1],
            cutlevels[cx + 1][cy + 1], xrem, yrem);
    }
...
static void print_badbit_finish(void)
{
    if (bad_total) {
        fprintf(stderr,
            ...
            "%lu (%G%%) irreparable.\n",
            bad_total, USEDBITS, 100 * (double)(bad_total) / USEDBITS, 100 * (double)bad_01 / (bad_total),
            100 * (double)bad_10 / (bad_total), irreparable, 100 * (double)irreparable / (bad_total));
    } else
        fprintf(stderr, "No bad bits!\n");
    fprintf(stderr,
        "BCH stats\n"
        "=========\n"
        "0 bad bits      %lu\n"
...
static void bch_bad_bits(uint64_t right, uint64_t wrong, unsigned long symno)
{
    int bit; /* FEC_LARGEBITS-1 = MSB, 0 = LSB */
    uint64_t diff = right ^ wrong;

    for (bit = FEC_LARGEBITS - 1; bit >= 0; bit--) {
        if ((diff >> bit) & 1ULL) {
            print_badbit(symno, (FEC_LARGEBITS - 1) - bit, (unsigned)((wrong >> bit) & 1ULL));
        }
    }
}

static uint64_t unbch(uint64_t in, unsigned long symno)
{
    struct bch_decoded dec = bch_decode(in);

    if (!dec.reparable) {
        int badbit;
        fputc('\n', stderr);
        for (badbit = 0; badbit < FEC_LARGEBITS; badbit++)
            print_badbit(symno, badbit, 2);
        fprintf(stderr, "!\n");
        irreparable += 4;
        bad_total += 4;
        golay_stats[4]++;
        return in >> FEC_SMALLBITS;
    }
    if (dec.errors > 0) {
        bch_bad_bits(dec.corrected, in, symno);
    }
    if (dec.errors >= 0 && dec.errors <= 3) {
        golay_stats[dec.errors]++;
    }
    return dec.data;
}
...
static void read_hamming_bit(unsigned char input, unsigned long symno)
{
    ...
    if (accubits >= FEC_LARGEBITS) {
        uint64_t mask = (((uint64_t)1) << FEC_LARGEBITS) - 1;
        uint64_t data = unbch(accu & mask, symno);
        for (shift = FEC_SMALLBITS - 1; shift >= 0; shift--)
            read_payload_bit((unsigned char)((data >> shift) & 1ULL));
        accu     = 0;
        accubits = 0;
    }
}
...
static void print_chan_info(void)
{
    ...
    fprintf(stderr, "net BCH payload capacity %G kB, ", (double)NETBITS / 8 / 1000);
    fprintf(stderr, "%lu BCH symbols, ", FEC_SYMS);
    fprintf(stderr, "%lu bits unused (incomplete BCH symbol), ", TOTALBITS - USEDBITS);
    ...
    fprintf(stderr, "border with crosses and BCH taking %G%% of unformatted capacity.\n",
...
static void blur_copy(void)
{
    ...
    if (!blur_cycles)
        memcpy(newary, ary, width * height);
    else
        fprintf(stderr, "\n");
}
...
static void process_minmax(void)
{
    ...
    if (npix)
        fprintf(stderr, "Doing %d cycles of max and %d cycles of min.\n", (int)npix, (int)npix);
...
static void fill(unsigned x, unsigned y, char test)
{
    ...
    while (que_read(&x, &y)) {
        if (x + 1 < width)
            try_copy_white(x + 1, y, test);
        if (x)
            try_copy_white(x - 1, y, test);
        if (y + 1 < height)
            try_copy_white(x, y + 1, test);
        if (y)
            try_copy_white(x, y - 1, test);
    }
}
...
void read_png(void)
{
    ...
    fprintf(stderr,
        "Input %u x %u pixels, taking %G megabytes for 2 "
        "framebuffers.\n",
...
static void parse_format(char* format)
{
    ...
    sscanf(format, "%u-%u-%u-%u-%u-%u-%u-%u", &dummy, &dummy, &dummy, &dummy, &dummy, &dummy, &dummy, &text_height);
